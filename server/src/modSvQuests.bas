        ...
        If Quest(QuestNum).RequiredLevel <= Player(index).Level Then
            ...
            For I = 1 To MAX_QUESTS_ITEMS
                If Quest(QuestNum).RequiredItem(I).Item > 0 Then
                    ...
                    If HasItem(index, Quest(QuestNum).RequiredItem(I).Item) = 0 Then
                        PlayerMsg index, "You need " & CheckGrammar(Trim$(Item(Quest(QuestNum).RequiredItem(I).Item).Name), 0) & " to start this quest.", BrightRed
                        Exit Function
                    End If
                End If
            Next
            ...
            If Quest(QuestNum).RequiredQuest > 0 And Quest(QuestNum).RequiredQuest <= MAX_QUESTS Then
                If Player(index).PlayerQuest(Quest(QuestNum).RequiredQuest).Status = QUEST_NOT_STARTED Or Player(index).PlayerQuest(Quest(QuestNum).RequiredQuest).Status = QUEST_STARTED Then
                    PlayerMsg index, "You need to complete the " & Trim$(Quest(Quest(QuestNum).RequiredQuest).Name) & " quest in order to start this one.", BrightRed
                    Exit Function
                End If
            End If
            'Go on :)
            CanStartQuest = True
        Else
            PlayerMsg index, "You need to be level " & Quest(QuestNum).RequiredLevel & " to take this quest.", BrightRed
        End If
    Else
        PlayerMsg index, "You can't start this quest again!", BrightRed
    End If
End Function
