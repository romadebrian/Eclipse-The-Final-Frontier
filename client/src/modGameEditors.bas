...
Dim F_CHAR As String * 1
    ...
    With Item(EditorIndex)
        ...
        frmEditor_Item.lblCombatType.Caption = "Combat Type: "
        frmEditor_Item.scrlCombatLvl.max = MAX_COMBAT_LEVEL
        frmEditor_Item.scrlCombatLvl = .CombatLvlReq
        frmEditor_Item.lblCombatLvl.Caption = "Combat Level: " & .CombatLvlReq
        ...
        If .Type = ITEM_TYPE_WEAPON Or .Type = ITEM_TYPE_SPELL Then
...
Dim value As Long
    ...
    With frmEditor_NPC
        ...
        .lblOutput.Caption = "Exp Varies: " & NPC(EditorIndex).EXP - (NPC(EditorIndex).EXP * mNum) & " - " & NPC(EditorIndex).EXP + (NPC(EditorIndex).EXP * mNum)
                    ...
                    Select Case tmpEvent.Pages(curPageNum).CommandList(curlist).Commands(i).Index
                        ...
                        Case EventType.evGiveExp
                            Select Case Abs(tmpEvent.Pages(curPageNum).CommandList(curlist).Commands(i).Data2)
                                Case 1
                                    frmEditor_Events.lstCommands.AddItem indent & "@>" & "Give Player " & tmpEvent.Pages(curPageNum).CommandList(curlist).Commands(i).data1 & " Combat Experience."
                                Case 0
                                    frmEditor_Events.lstCommands.AddItem indent & "@>" & "Give Player " & tmpEvent.Pages(curPageNum).CommandList(curlist).Commands(i).data1 & " [" & Trim$(Skill(tmpEvent.Pages(curPageNum).CommandList(curlist).Commands(i).Data3).name) & "] Experience."
                            End Select
